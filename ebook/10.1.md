...
刚接触这个术语的人总是容易误解，这里必须澄清的是，happens before并不是指时序关系，并不是说A happens before B就表示操作A在操作B之前发生。它就是一个术语，就像光年不是时间单位一样。详细地说：
