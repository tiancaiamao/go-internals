...
## 从本书的源代码安装
出于学习Go内部实现的目的，这里推荐的安装方式是从本书附带的源代码安装。本书使用的是标准的Go1.1发布的源代码，将这份源代码中的一些核心部分做了中文注释。有些时候可能会用调试器跟踪代码，这份带注释的源代码会比较方便。

下载本书源码

	git clone https://github.com/tiancaiamao/go-internals.git
	cd go-internals/go/src
	./all.bash
...
然后设置几个环境变量，
	...
	export GOROOT=$HOME/go-internals/go
	export PATH=$PATH:$GOROOT/bin

这里假设读者是从$HOME运行的上面命令。否则你要将GOROOT设置成相应的目录。如果你只是想对Go的内部实现有一些基本了解，但并不想对照源代码深入研究，可以继续往下阅读。
