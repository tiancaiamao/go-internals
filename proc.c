...
/* ´Óº¯ÊýÃû¾Í¿ÉÒÔ¿´³ö,Ëü¸ÉµÄÊÂÇéÊÇ,°Ñg±ä³ÉGrunnableµÄ,¹ÒÔÚ¾ÍÐ÷¶ÓÁÐ
   ÒªÇóµ÷ÓÃº¯ÊýÇ°µ÷¶ÈÆ÷ÊÇËø×ÅµÄ,ËùÒÔ½Ðreadylocked
 */
static void
readylocked(G *g)
{
...
/* ×î¶àÍ¬Ê±Ö»ÓÐGOMAXPROCS¸ögÕýÔÚÔËÐÐ,´¦ÓÚÏµÍ³µ÷ÓÃµÄ²»Ëã
   Õû¸öº¯Êý¾ÍÊÇ·µ»Øm->nextg,¼´ÏÂÒ»¸ö´ýÔËÐÐµÄgoroutine
 */
static G*
nextgandunlock(void)
{
...
top:
	...
	/* Èç¹ûµ±Ç°µÄmÖÐÓÐÕýÔÚµÈ´ý´¦ÀíµÄg,Ôò´¦Àí¸Ãg
	 */
	if(m->nextg != nil) {
		gp = m->nextg;
		m->nextg = nil;
	...
	if(m->lockedg != nil) {
		...
		// Wait on global m queue.
                /* ¾ÍÐ÷g¶ÓÁÐÖÐ¶¼Ã»ÓÐÁË»òÕß²»ÄÜ¼ÓmcpuÁË,Ôò°Ñm·Åµ½¶ÓÁÐÖÐ
		   ºóÃæÔÙ·ÖÅäm,¼´newmº¯ÊýÖÐ,»áÏÈ´Ó¶ÓÁÐÄÃ.Ã»ÓÐ²Å»áÔÙ·ÖÅä
		 */
		mput(m); 
	}
...
runtime.mstart(void)
{
	/* mstartÊÇruntime.newosprocÐÂ½¨µÄÏß³ÌµÄÈë¿ÚµØÖ·
	   ÐÂÏß³ÌÖ´ÐÐÊ±»á´ÓÕâÀï¿ªÊ¼ÔËÐÐ
	   ×¢ÒâÔÚruntime.newosprocÖÐ´«¹ýÀ´Ê±,²ÎÊý¾ÍÊÇg0ºÍg0µÄÕ»
	   ²¢²»ÊÇÐÂ½¨goroutineµÄg!
	 */
	if(g != m->g0)
	...
	/* ¼ÇÂ¼ÏÂÕ»¶¥¹©mcallÊ¹ÓÃ.
	   Ò»µ©µ÷ÓÃÁËscheduleº¯Êý,schedule²»»á¼ÌÐøÊ¹ÓÃÕâ¸öÕ»(schedule²»ÊÇÆÕÍ¨µÄÕ»µ÷ÓÃ·½Ê½,±»µ÷Õß¼ÌÐøÊ¹ÓÃµ÷ÓÃÕßÕ»µÄÏÂÃæµÄµØÖ·¿Õ¼ä)
	   Òò´ËÆäËüµ÷ÓÃ¿ÉÒÔÖØÓÃÕâ¸öÕ»µØÖ·¿Õ¼ä
	 */
	runtime.gosave(&m->g0->sched);
	m->g0->sched.pc = (void*)-1;  // make sure it is never used
	runtime.asminit();
	runtime.minit(); /* ÎªÐÅºÅ´¦Àí½¨Á¢ÁË×¨ÃÅµÄG½á¹¹Ìå */
...
// Sched is locked.
/* Õâ¸öº¯Êý¾ÍÊÇ×ö¸öÆ¥Åä,Ö»ÒªmÃ»ÓÐÍ»ÆÆÉÏÏÞ,¾ÍÐ÷¶ÓÁÐÖÐ»¹ÓÐg,¾ÍÓÃÒ»¸ömÔËÐÐÒ»¸ög */
static void
matchmg(void)
{
	G *gp;
	...
	if(m->mallocing || m->gcing)
		return;
	/* Ö»Òª¾ÍÐ÷¶ÓÁÐÖÐÓÐg,²¢ÇÒ¿ÉÔËÐÐµÄmÊýÃ»ÓÐµ½ÉÏÏÞ */
	while(haveg() && canaddmcpu()) {
		gp = gget();
		if(gp == nil)
			runtime.throw("gget inconsistency");
		...
		if((mp = mget(gp)) == nil)  //mµÄwaiting¶ÓÁÐÖÐÓÐ¾ÍÖ±½ÓÄÃ,Ã»ÓÐ¾ÍÐÂ½¨
			mp = runtime.newm();
		mnextg(mp, gp);
	}
}
...
/* ÆäÊµ¾ÍÊÇÐÂ½¨Ò»¸ö²Ù×÷ÏµÍ³Ïß³Ì,Ïß³ÌµÄÈë¿ÚµãÊÇmstart
...
runtime.newm(void)
{
	...
	mcommoninit(m);  //×öÒ»Ð©³õÊ¼»¯¹¤×÷

        /* cgoÊÇ²»ÄÜÔÚ·Ö¶ÎÕ»ÔËÐÐ,ÒªÇÐ»»µ½ÏµÍ³Õ»ÖÐ,ËùÒÔ´úÂë´¦ÀíÓÐÐ©²»Í¬
	   Õâ²¿·ÖÏÈÌø¹ý²»¿´
	 */
	if(runtime.iscgo) {  
		CgoThreadStart ts;
...
schedule(G *gp)
{
	...
	if(gp != nil) {
		...
		case Grunning: 
			...
			break;
		case Gmoribund:
			/* ÉèÖÃgµÄ×´Ì¬ÎªGdead,½«gÓëm·ÖÀë,°Ñg·Å»Øµ½free¶ÓÁÐ */
			gp->status = Gdead;
			if(gp->lockedm) {
				gp->lockedm = nil;
				m->lockedg = nil;
		...
		/* exitsyscallÊ±Èç¹ûmcpu´óÓÚÉÏÏÞÁË,Ôòg³öÁËÏµÍ³µ÷ÓÃ²¢²»ÄÜ¼ÌÐøÔËÐÐ,ÒªÍ¨¹ýµ÷ÓÃgosched»á½øÈëµ½schedule
		   ÕâÖÖÇé¿ögÊÇ±»ÉèÖÃÁËreadyonstopµÄ,½«Ö®·ÅÈë¾ÍÐ÷¶ÓÁÐ
		 */
		if(gp->readyonstop){
			gp->readyonstop = 0;
			readylocked(gp);
	...
	/* ÏÂÃæÕâ¶Î´úÂë,ÕÒ¸ö´ýÔËÐÐµÄg,½«Ëü°áµ½m->curg,ÉèÖÃÆä×´Ì¬ÎªGrunning
	   ÕÒ´ýÔËÐÐµÄgÏÈ¿´µ±Ç°µÄm¼Ä´æÆ÷µÄnextg,ºÍlockedgÓò,Èç¹ûÓÐ¾ÍÊÇËüÃÇ
	   Èç¹ûÃ»ÓÐ,ÔÙÈ¥¾ÍÐ÷g¶ÓÁÐÖÐÕÒ.
	...
	/* ²é¿´gµÄÉÏÏÂÎÄ»·¾³ÖÐ±£´æµÄpc¼Ä´æÆ÷,Èç¹ûÊÇruntime.goexit,ËµÃ÷Ïß³ÌÖ´ÐÐÍêÁË
	   Òò´Ëµ÷ÓÃÏàÓ¦µÄÍË³ö´¦Àí.·ñÔò»Ö¸´µ½gµ±Ê±µÄÉÏÏÂÎÄ
	   runtime.gogo¾ÍÏàµ±ÓÚCÓïÑÔµÄÒ»¸ölongjmp,²»ÊÇº¯Êýµ÷ÓÃÐÎÊ½,¶øÊÇÖ±½ÓÇÐ»»ÉÏÏÂÎÄ
	 */
...
/* µ±goroutine½øÈëµ½syscallÖ®ºómcpuÊýÁ¿»á¼õ.ÕâÑùÔÚmatchmgÖÐ,¾Í¿ÉÒÔÐÂ·ÖÅämÀ´ÔËÐÐ¾ÍÐ÷µÄg.
   µ«ÊÇÎÒÓÐ¸öÒÉÎÊ:g½øÈësyscallºó²¢Ã»ÓÐÓëm°þÀë,Èç¹ûmatchmgÖÐnewmÁË,ÄÇÃ´Æñ²»ÊÇ»áÒýÈë¸ü¶àµÄosÏß³ÌÁË??
   ËäÈ»¿ÉÒÔ½¨ºÜ¶àgoroutineÎÞËùÎ½(Ã¿¸öÖ»ÊÇ4kµÄÕ»,²¢ÇÒÓëÏµÍ³Ïß³ÌÎÞ¹Ø),µ«ÊÇÈç¹û½¨³öÀ´µÄgoroutine²»Í£µØentersyscall,
...
runtime.entersyscall(void)
{
	...
	/* entersyscall×öµÄÊÂÇé´óÖÂÊÇÉèÖÃgµÄ×´Ì¬ÎªGsyscall,¼õÉÙmcpu
	   Èç¹ûmcpu¼õÉÙÖ®ºóÐ¡ÓÚmcpumaxÁË²¢ÇÒÓÐ´¦ÓÚ¾ÍÐ÷Ì¬µÄg,Ôòmatchmg
	 */
	...
	schedlock();
	/* ½øÐÐµ½ÕâÀïËµÃ÷mcpu--ÁËÖ®ºóÊÇÐ¡ÓÚmcpumaxµÄ,Òò´ËÈç¹ûÓÐ¾ÍÐ÷µÄg,Ôòmatchmg¸øÓèÔËÐÐ»ú»á
	 */
	v = runtime.atomicload(&runtime.sched.atomic);
	...
	schedunlock(); /* unlockÖ®ºóÔËÐÐµÄ¾Í²»ÔÙÊÇÕâ¸ömÁË */
}
...
/* Èç¹ûmcpuÐ¡ÓÚmcpumaxµÄÇé¿ö,Ö±½ÓreturnÁË,²¢ÇÒgµÄ×´Ì¬ÊÇrunning.±íÊ¾ÈÃËü¼ÌÐøÔËÐÐ
   Èç¹ûmcpuÉÏÏÞÁË,ÔòÉèÖÃreadyonstop,ÏÂÒ»´ÎscheduleÖÐ½«Ëü¸Ä³ÉGreadyÁË·Åµ½¾ÍÐ÷¶ÓÁÐÖÐ
 */
void
...
runtime.malg(int32 stacksize)
{
	...
	if(stacksize >= 0) {
		/* m->g0ÊÇµ÷¶ÈÆ÷ËùÔÚµÄÕ»µÄgoroutine,m->curgÊÇmÖÐµ±Ç°ÔËÐÐ×ÅµÄg
		   gÊÇÒ»¸ö¼Ä´æÆ÷,ÊÇµ±Ç°ÔËÐÐµÄgoroutineµÄ¼Ä´æÆ÷
		   ·ÖÅäÒ»¸öÐÂÕ»¿Õ¼äµÄ´úÂëÒªÇÐ»»µ½µ÷¶ÈÆ÷Õ»È¥ÔËÐÐ
		 */
		if(g == m->g0) {
			// running on scheduler stack already.
			/* stackallocÔÚÎÄ¼þmalloc.gocÖÐ,ÉÏÃæÓÐ×¢ÊÍËµËü±ØÐëÔÚµ÷¶ÈÆ÷Õ»ÉÏ±»µ÷ÓÃ,
			   ÕâÑù¾Í²»±ØÔÚstackallocËùÔËÐÐµÄÕ»ÉÏÔö³¤Õ»
			 */
...
/* º¯Êý¹¦ÄÜ:´´½¨Ò»¸öÐÂµÄg
   Õâ¸öº¯Êý²»ÄÜÓÃ·Ö¶ÎÕ»,ÕæÕýµÄ¹¤×÷ÊÇµ÷ÓÃnewproc1Íê³ÉµÄ
   newproc1µÄ¶¯×÷°üÀ¨
   ·ÖÅäÒ»¸ögµÄ½á¹¹Ìå
   ³õÊ¼»¯Õâ¸ö½á¹¹ÌåµÄÒ»Ð©Óò
   ½«Ëü¹ÒÔÚ¾ÍÐ÷¶ÓÁÐ
   Òý·¢Ò»´Îµ÷¶Èmatchmg
 */
#pragma textflag 7
...
//printf("newproc1 %p %p narg=%d nret=%d\n", fn, argp, narg, nret);
	...
	/* ÏÂÃæÕâÒ»¶Î×öµÄÊÂÇéÊÇ·ÖÅäÒ»¸ögµÄ¿Õ¼ä
	   Èç¹ûgfree¶ÓÁÐÖÐÓÐ¾ÍÖ±½ÓÈ¡,Ã»ÓÐ
	   ¾ÍÔÙµ÷ÓÃmalgº¯Êý·ÖÅäÒ»¸öÐÂµÄg,²¢¹ÒÔÚallg¶ÓÁÐÎ²
	...
	/* ÏÂÃæÕâÒ»¶Î×öµÄÊÂÇéÊÇ×öÒ»Ð©³õÊ¼»¯gµÄ¹¤×÷
	   °üÀ¨°Ñµ÷ÓÃ²ÎÊýÒÆµ½gµÄÕ»ÖÐ
	   °ÑgµÄsp,pcµÈ½ø³ÌÉÏÏÂÎÄ±£´æÔÚgµÄschedÖÐ
	...
	newg->sched.sp = sp;
        /* ×¢Òâµ½ÕâÀïµÄ·µ»ØµØÖ·ÊÇgoexit,ÔÚscheduleÖÐ»Ö¸´goroutineÉÏÏÂÎÄ»·¾³Ê±,
	...
	/* Õâ¸öº¯Êý½«°Ñnewg×´Ì¬ÉèÖÃ³ÉGrunnable,²¢ÒýÆðÒ»½«µ÷¶Èmatchmg */
